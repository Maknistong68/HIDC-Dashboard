...
/**
 * HazardRootCauseChart - Horizontal bar chart showing top contributing factors
...
const HazardRootCauseChart = ({ rootCauses = [], onBarClick, isMobile = false }) => {
  const { chartData, maxCount } = useMemo(() => {
    ...
    const data = rootCauses.map(rc => ({
      name: rc.name.length > 18 ? rc.name.substring(0, 18) + '...' : rc.name,
      fullName: rc.name,
      count: rc.count,
    ...
    return (
      <div className={`bg-white rounded-lg border border-surface-200 ${isMobile ? 'p-3' : 'p-4'}`}>
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs font-semibold text-surface-500 uppercase tracking-wide">Contributing Factors</span>
        </div>
        <p className="text-sm text-surface-400 text-center py-6">No root cause data available</p>
      </div>
    )
  }
    ...
    <div className={`bg-white rounded-lg border border-surface-200 ${isMobile ? 'p-3' : 'p-4'}`}>
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-surface-500 uppercase tracking-wide">Top Contributing Factors</span>
        </div>
        <span className="text-2xs text-surface-400">{rootCauses.length} factors</span>
      </div>
