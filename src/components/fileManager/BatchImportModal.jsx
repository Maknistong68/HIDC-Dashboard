 ...
 *
 * This modal is rendered at the app root level by ImportLockContext when locked.
 * It CANNOT be unmounted during processing - all navigation is blocked.
 *
 * Key guarantees:
 * - No isMountedRef checks - modal stays mounted
 * - No abort logic - processing must complete
 * - Errors are caught per-file, processing continues
...
const BatchImportModal = ({ onClose, onProcessingStart, onProcessingEnd }) => {
  ...
  // Process all files - GUARANTEED TO COMPLETE
  // No abort checks, no mount checks - the lock system ensures we stay alive
  const processFiles = useCallback(async (filesToProcess, existingIncidents) => {
    ...
    // Process EVERY file - no early exits
  ...
  const handleDone = useCallback(async () => {
    ...
    try {
      ...
      if (import.meta.env.DEV) {
        console.error('[BatchImport] Error reloading files:', error)
      }
    } finally {
  ...
  // Modal is rendered inline (ImportLockContext handles the portal)
  return (
    <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[85vh] flex flex-col animate-scale-in">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {!isComplete ? (
          <>
            {/* Drop zone */}
            <div
              className={`border-2 border-dashed border-surface-300 rounded-lg p-6 text-center transition-colors mb-4 ${
                isProcessing
        ...
        {!isComplete ? (
          <>
            <button
              onClick={handleCancel}
              disabled={isProcessing}
              className="px-4 py-2 text-surface-700 font-medium hover:bg-surface-100 rounded-lg transition-colors disabled:opacity-50 disabled:pointer-events-none"
            >
              Cancel
            </button>
            <button
