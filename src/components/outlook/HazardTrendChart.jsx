...
import { TrendingUp, TrendingDown, Minus, Target, ZoomIn, ZoomOut, Maximize2, Move } from 'lucide-react'

/**
 * Calculate moving average for an array of data points
 */
const calculateMovingAverage = (data, windowSize = 7, key = 'negative') => {
...
const TradingTooltip = ({ active, payload, label, activeDays, avgValue }) => {
  if (active && payload && payload.length) {
    ...
    const negativeValue = payload.find(p => p.dataKey === 'negative')?.value || 0
    const maValue = payload.find(p => p.dataKey === 'movingAvg')?.value

    const vsAvg = avgValue > 0 ? ((negativeValue - avgValue) / avgValue * 100).toFixed(0) : 0
    const isAboveAvg = negativeValue > avgValue
    ...
    return (
      <div className="bg-surface-900 text-white px-4 py-3 rounded-lg shadow-xl border border-surface-700 min-w-[160px] pointer-events-none">
        ...
        <div className="flex items-baseline justify-between mb-1">
          <span className="text-xs text-surface-400">Observations</span>
          <span className="text-lg font-bold text-red-400">{negativeValue}</span>
        </div>
...
/**
 * HazardTrendChart - Trading-style chart with drag & scroll zoom
 */
const HazardTrendChart = ({ data, hazardName, timePeriod }) => {
  const [showMovingAvg, setShowMovingAvg] = useState(true)
  const [viewRange, setViewRange] = useState({ start: 0, end: null }) // null = end of data
  ...
  // Calculate average for reference line (based on visible data)
  const avgValue = useMemo(() => {
    if (visibleDays.length === 0) return 0
    const sum = visibleDays.reduce((acc, d) => acc + (d.negative || 0), 0)
  ...
  // Y-axis max (dynamic based on visible data)
  const yAxisMax = useMemo(() => {
    if (visibleDays.length === 0) return 1
    const maxCount = Math.max(...visibleDays.map(d => d.negative || 0), 1)
  ...
  const zoom = useCallback((direction, centerRatio = 0.5) => {
    ...
    let newRange
    if (direction === 'in') {
      newRange = Math.max(Math.floor(currentRange * 0.7), 5) // Min 5 data points
    } else {
      newRange = Math.min(Math.floor(currentRange * 1.4), allDays.length - 1)
    }

    // Calculate new start/end based on center point
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    const deltaX = dragStart - e.clientX // Inverted for natural panning

    // Convert pixel delta to data points
  ...
  if (!data || !data.hasData) {
    return (
      <div className="flex flex-col items-center justify-center h-80 text-center p-4">
        ...
        <p className="text-xs text-surface-400 mt-1">Select a hazard to view its trend</p>
      </div>
    )
  }
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
        <div>
          <h3 className="text-base font-semibold text-surface-800">{hazardName}</h3>
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          <div className="flex items-center gap-1.5 bg-surface-100 rounded-full px-2 py-1">
            ...
            <span className="font-bold text-red-600">{totalNegative || 0}</span>
          </div>
          <div className="flex items-center gap-1.5 bg-surface-100 rounded-full px-2 py-1">
      ...
      {/* Trading-style chart with drag & scroll */}
      <div
        ref={chartRef}
      ...
      {visibleDays.length === 0 && (
        <p className="text-xs text-surface-400 text-center mt-2">No observations in this period</p>
      )}
    </div>
  )
}

export default React.memo(HazardTrendChart)
