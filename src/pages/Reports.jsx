...
const ReportCard = ({ title, description, icon: Icon, onClick }) => (
  <div
    onClick={onClick}
    className="bg-white rounded-xl border border-surface-200 p-6 hover:shadow-md cursor-pointer transition-shadow"
  >
...
const MonthlyReport = ({ projects, incidents, engagements, compliance, month }) => {
  ...
  return (
    <div className="bg-white rounded-xl border border-surface-200 p-6 print:border-none print:p-0">
      <div className="text-center mb-8 print:mb-4">
        <h1 className="text-2xl font-bold text-surface-900">
          Monthly HSE Summary Report
        </h1>
        <p className="text-surface-500">{format(month, 'MMMM yyyy')}</p>
      </div>

      {/* Key Metrics */}
      <section className="mb-8">
        <h2 className="text-lg font-semibold text-surface-900 mb-4 border-b pb-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-surface-50 rounded-lg p-4 text-center">
            ...
            <p className="text-sm text-surface-500">TRIR</p>
          </div>
          <div className="bg-surface-50 rounded-lg p-4 text-center">
            <p className="text-2xl font-bold text-surface-900">
              {calculateDaysWithoutLTI(incidents)}
            </p>
            <p className="text-sm text-surface-500">Days Without LTI</p>
          </div>
          <div className="bg-surface-50 rounded-lg p-4 text-center">
            <p className="text-2xl font-bold text-surface-900">{monthIncidents.length}</p>
            <p className="text-sm text-surface-500">Total Incidents</p>
          </div>
          <div className="bg-surface-50 rounded-lg p-4 text-center">
            <p className="text-2xl font-bold text-surface-900">{monthEngagements.length}</p>
            <p className="text-sm text-surface-500">Total Engagements</p>
          </div>
        </div>
      </section>
      ...
      {/* Footer */}
      <div className="text-center text-sm text-surface-500 border-t pt-4">
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 no-print">
        <div>
          ...
          <p className="text-surface-500">Generate and export safety reports</p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={handleExportJSON}
            className="flex items-center gap-2 px-4 py-2 border border-surface-200 text-surface-700 rounded-lg hover:bg-surface-50 font-medium transition-colors"
          >
            <Download size={20} />
            Export All Data
          </button>
      ...
      {!selectedReport ? (
        <>
          ...
          <div className="bg-white rounded-xl border border-surface-200 p-6">
            <h3 className="font-semibold text-surface-900 mb-4">Quick Overview</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center p-4 bg-surface-50 rounded-lg">
                <p className="text-3xl font-bold text-surface-900">{projects.filter(p => p.status === 'active').length}</p>
                <p className="text-sm text-surface-500">Active Projects</p>
              </div>
              <div className="text-center p-4 bg-surface-50 rounded-lg">
                <p className="text-3xl font-bold text-surface-900">{incidents.length}</p>
                <p className="text-sm text-surface-500">Total Incidents</p>
              </div>
              <div className="text-center p-4 bg-surface-50 rounded-lg">
                <p className="text-3xl font-bold text-surface-900">{engagements.length}</p>
                <p className="text-sm text-surface-500">Total Engagements</p>
              </div>
              <div className="text-center p-4 bg-surface-50 rounded-lg">
                <p className="text-3xl font-bold text-surface-900">{compliance.length}</p>
                <p className="text-sm text-surface-500">Compliance Items</p>
              </div>
            </div>
          </div>
        </>
          ...
          <div className="flex flex-wrap items-center gap-4 no-print">
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                className="px-3 py-2 border border-surface-200 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none"
              />
            </div>
